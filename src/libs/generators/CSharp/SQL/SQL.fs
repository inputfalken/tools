...
let toSqlType (str: string): Sql.Types.DataType =
    let ciString =
        str.Replace("System.", String.Empty, StringComparison.OrdinalIgnoreCase)
        |> CIString.CI

    match ciString with
    | x when x.Equals int
             || x.Equals int32
             || x.Equals intNullable
             || x.Equals int32Nullable -> Int
    | x when x.Equals bool
             || x.Equals boolean
             || x.Equals boolNullable
             || x.Equals booleanNullable -> Bit
    | x when x.Equals guid || x.Equals guidNullable -> UniqueIdentifier
    | x when x.Equals dateTime || x.Equals dateTimeNullable -> DateTime
    | x when x.Equals float
             || x.Equals double
             || x.Equals floatNullable
             || x.Equals doubleNullable -> Float
...
let parseClass (input: string) =
    let res =
        ...
        |> CSharpExtensions.GetCompilationUnitRoot
...
let generateStoredProcedureFromCSharp (cSharp: string) (settings: Settings): string =
    ...
    | GenerationType.UserDefinedTableType ->
        ...
        [ f ]
        |> List.map ProcedureParameter.UserDefinedTableType
